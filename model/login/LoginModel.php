...
class LoginModel{
	...
	public function __construct() {
		$this->UserD = new MyMongo('user');

	}

	public function login($uname,$passwd,$r)
	{
		if( $uname == "" || $passwd == "")
		{
			return false;
		...
		$this->UserD->setCollection('user');

		$cursor = $this->UserD->findOne(array('status'=>1,'username'=>$uname,'passwd'=>$passwd));
		if($cursor)
		{
			if($r ==1)
			{
				self::setSessionAndCookie(self::mongoDoc2Array($doc));
			}
			else{
				self::setSession(self::mongoDoc2Array($doc));
			}
		}
